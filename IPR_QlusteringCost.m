function [Cost,J]=IPR_QlusteringCost(H_mat,k,n,q,gammain,Lin2,Ldep,Lout,psi,boundaries)
    ...
    J=J_vec;
    %% Cost Function.
    in1 = J(1,:) >= boundaries(1) & J(1,:) <= boundaries(2);
    in2 = J(2,:) >= boundaries(1) & J(2,:) <= boundaries(2);
    
    J_tags = 0.5 * ones(size(J));
    
    J_tags(:, in1 & ~in2) = repmat([1; 0], 1, sum(in1 & ~in2));
    J_tags(:, in2 & ~in1) = repmat([0; 1], 1, sum(in2 & ~in1));
    
    Cost = sum(sum((J_tags - J).^2));
end
